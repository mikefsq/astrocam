...
// writeFITS writes one image as a FITS primary HDU.
//
// RAW16 samples are little-endian unsigned; FITS BITPIX=16 is signed big-endian, so the
// file uses the unsigned-16 convention: store (v - 32768) as signed and advertise
// BZERO=32768/BSCALE=1. Per sample that is a byte swap plus XOR 0x80 on the high byte.
//
// data is width*height*bpp bytes. bayer is "" for mono, else the CFA pattern (e.g. "RGGB").
// Rows are written top-first with ROWORDER=TOP-DOWN.
func writeFITS(path string, data []byte, width, height, bpp int, bayer string, exposureSec, pixUm float64, gain int, model string) error {
	...
	// RAW8: BITPIX 8, no BZERO/BSCALE. RAW16: BITPIX 16 with BZERO=32768.
	bitpix := 16
	if bpp == 1 {
		bitpix = 8
	}
	...
	if bayer != "" {
		// The mosaic phase of a sub-frame is the phase at its window origin, and every profile
		// aligns that origin to an even sensor pixel on both axes (Sensor.ROIStartAlign, pinned
		// by TestROIStartAlignIsEven), so a window never shifts the pattern against BAYERPAT.
	...
	if bpp == 1 {
		// RAW8 bytes go out verbatim.
		if _, err := w.Write(data); err != nil {
			return err
		}
	} else {
		// RAW16: swap to big-endian and flip the sign bit, in 1 MiB chunks.
		tmp := make([]byte, 1<<20)
...
// fitsCard builds an 80-byte header card: KEYWORD (<=8, left-justified), "= ", the value
// field, an optional " / comment", space-padded to 80.
...
// trim formats a float in its shortest fixed-point form for the header. FITS writes an exponent
// with E or D, and %g would switch to Go's "e" form below 1e-4, which a 50 µs exposure reaches.
