...
// burst), tools.go (bench tools: -list, -thermal, -dumpflash, -tecoff, -heater, -cool,
// -regulate, -replay, -soak, -wedge), logt.go (the -v transport logger), fits.go, ser.go.
package main
...
func main() {
	...
	usb2 := flag.Bool("usb2", false, "force the USB2 readout path regardless of model/link")
	fpsPerc := flag.Int("fps", 0, "FPS / bandwidth percent 40..100 (0 = the link default: 100 on USB3, 40 on USB2)")
	...
	fixDefects := flag.Bool("fixdefects", false, "with a single-frame -capture: apply the factory defect map to a full-frame RAW16 capture")
...
func openCamera(tg target) (astrocam.Transport, *astrocam.Camera, target, error) {
	...
	cam, err := astrocam.Open(raw, tg.vid, tg.pid)
	if err != nil {
		raw.Close()
...
type captureOpts struct {
	...
	fpsPerc    int  // FPS / bandwidth percent (40..100); 0 = bus default
