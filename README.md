...
The driver is cgo-free everywhere except the macOS USB backend. Linux and Windows build with
...
IMX571 profiles. The per-vendor differences are the register-map opcodes (`Regmap`) and the
gain/offset unit scale, selected at call time from the regmap's VID. Vendors register themselves
...
Only macOS, Linux and Windows have a USB backend. Other platforms compile (the stub transport
and tests work there), and `OpenHost`/`OpenLocation`/`Enumerate` return an error.
...
- **ASI462MC (IMX462)**: 12-bit and 10-bit high-speed; shares the FX3 frame-marker repair (a
  per-sensor opt-in, so unverified sensors are never touched).
- **ASI174MM Mini (IMX174)**: global shutter, no hardware binning.

Decoded but not yet validated on hardware:
...
- **PlayerOne models** (Apollo, Sedna, Mars/Ceres, Zeus, Poseidon, Uranus): registered over the
  shared profiles through the PlayerOne regmap dialect; no PlayerOne camera has been driven yet.
...
On Linux, opening the device needs udev access to the camera's USB vendor id (ZWO `0x03C3`,
PlayerOne `0xA0A0`).

## gosnap

`cmd/gosnap` is the bring-up and capture tool. The default mode detects the camera, connects,
prints its identity, and disconnects (control transfers only). `-capture` runs the full init and
a single exposure (the first-light test). `-v` logs every USB transfer, for debugging an arm
sequence against the real device. `-vid`/`-pid` select the camera (defaults `0x03c3`, ZWO, and
`0x1749`, the ASI174MM Mini); `-list` enumerates attached cameras with their serials.
